...
bool fq__msg__actor_zones__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("fq.msg._actor_zones.ActorZones", full_classname_dest, 30) == 0);
  }
  fq__msg__ActorZones * ros_message = _ros_message;
  {  // header
  ...
  {  // zones
    ...
    if (!fq__msg__ActorZone__Sequence__init(&(ros_message->zones), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create fq__msg__ActorZone__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    fq__msg__ActorZone * dest = ros_message->zones.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!fq__msg__actor_zone__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
{
  /* NOTE(esteve): Call constructor of ActorZones */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("fq.msg._actor_zones");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "ActorZones");
  ...
  {  // zones
    ...
    fq__msg__ActorZone * item;
    for (size_t i = 0; i < size; ++i) {
      item = &(ros_message->zones.data[i]);
      PyObject * pyitem = fq__msg__actor_zone__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
