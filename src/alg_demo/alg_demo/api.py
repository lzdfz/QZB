...
class FqTopic(Enum):
    ...
    # 蜂群参数
    dron_swarm_param = "fq/dron_swarm"

    # 蜂群控制返回值
    dron_swarm_control = "fq/dron_swarm_control"

    # 蜂群打击返回值
    dron_swarm_hit_info = "fq/dron_swarm_hit_info"
...
FqTopic2Message = {
    FqTopic.dron_swarm_param: ActorDroneSwarms,
    FqTopic.dron_swarm_control: ActorControlInfos,
    FqTopic.dron_swarm_hit_info: ActorHitInfo,
...
class SimMessageNode(Node):
    ...
    def __init__(self):
        ...
        self._subscription_handler = {
            FqTopic.dron_swarm_param: SimMessageNode.TopicSubscriptionHandler(self, FqTopic.dron_swarm_param),
        ...
        self._control_command_publisher = self.create_publisher(
            FqTopic2Message.get(FqTopic.dron_swarm_control),
            FqTopic.dron_swarm_control.value,
            10
        )

        self._hit_publisher = self.create_publisher(
            FqTopic2Message.get(FqTopic.dron_swarm_hit_info),
            FqTopic.dron_swarm_hit_info.value,
            10
        )
...
class API(object):
    ...
    @staticmethod
    def next_dron_swarm_param():
        return API.next_data(FqTopic.dron_swarm_param)
